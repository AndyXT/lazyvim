...
function M.setup(opts)
  ...
  -- Allow setting the default JSON file path
  if opts.default_json_path then
    JSON_FILE_PATH = opts.default_json_path
  end
  
  -- Allow customizing the command name
  local cmd_name = opts.command_name or "CommandRunner"
  
  -- Create the command with the configured name
  vim.api.nvim_create_user_command(cmd_name, function(cmd_opts)
    my_fancy_command(cmd_opts)
  end, {
    nargs = "?",
    desc = "Run commands from a JSON file (accepts optional file path)",
    complete = "file"
